...
public class SwordProceduralAnimator : MonoBehaviour
{
    ...
    [Tooltip("拖 PlayerCombatController 或 EnemyAIController 进来，只要实现 IAttackSource 就行")]
    ...
    // ---------- 基础旋转 ----------
    private Quaternion _baseLocalRotation;   // 初始局部旋转（所有程序化旋转都叠加在这上面）
    ...
    private void OnEnable()
    {
        ...
        {
            Debug.LogWarning("[SwordProceduralAnimator] attackSourceComponent 必须实现 IAttackSource", this);
            return;
        }
    ...
    private void ApplyAngles(float yaw, float pitch, float roll)
    {
        ...
        // 叠加顺序：先 yaw 再 pitch 再 roll（可以按感觉调整）
